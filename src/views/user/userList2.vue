<template>
  <div>
    <span>用户管理2</span>
    <table-page :search-json="searchJson" :table-json="tableJson" :dialog-json="dialogJson">
      <template #gender="{row}">
        {{ $allData.common.gender[row.gender] }}
      </template>
      <template #active="{row}">
        <el-button type="text" size="small" @click="handleUpdata('open', row)">编辑</el-button>
        <el-button type="text" size="small" @click="handleDelete(row._id)">删除</el-button>
      </template>
      <template #dialogEdit>
  ...
  methods: {
    handleQuery(params) {
      ...
      this.Http.post("user.userList", this.searchParams).then(data => {
        ...
        this.tableJson.total = data.data.length;
      });
    },
    handleUpdata(type, data) {
      if (data) {
        this.form = { ...data };
